...
ckpt_path = './tl_checkpoints/tl_stem_root_epoch=399-step=6800.ckpt' 

parent_dir = "tb_logs/complete"
...
if __name__ == "__main__":
    species_logger = TensorBoardLogger(parent_dir, name="tl_stem_root_for_species_part")
    ...
    dmcot_logger = TensorBoardLogger(parent_dir, name="tl_stem_root_for_dmcot")
    train_dm, test_dm = get_loaders(path_to_train_dicot_monocot, path_to_test_dicot_monocot)
    model_dm = TransferClassifier(num_classes=2, task_name="dicot_monocot", checkpoint_path=ckpt_path)
    
    trainer_dm = pl.Trainer(
        max_epochs=max_epochs_classifier, 
        accelerator="gpu", 
        devices=1, 
        logger=dmcot_logger, 
    ...
    trainer_dm.fit(model_dm, train_dm, test_dm)